    ...
    <>
      <div className="flex gap-[14px]">
        ...
        <div>
          ...
          <p className={`${data?.status === "successful"? "text-[#0EA163]":"text-[#56616B]"} ${data?.status === "pending"? "text-[#A77A07]":"text-[#56616B]"} text-sm font-medium capitalize`}>{data?.status}</p>
        </div>
      </div>
      <div className="text-right">
        <p className="text-[#131316] font-bold text-base">USD {data.amount}</p>
        <p className="text-[#56616B] text-sm font-medium">{formatDate(data.date)}</p>
      </div>
    </>
  );
};
